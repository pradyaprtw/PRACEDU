...
@section('content')
    <h1 class="text-2xl font-semibold text-gray-700 mb-6">Tambah Sub-Kategori Baru</h1>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('admin.sub-categories.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('name') border-red-500 @enderror" placeholder="Contoh: Kelas 10, Saintek, Soshum">
                @error('name')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
