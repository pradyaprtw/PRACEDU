...
@section('content')
    <h1 class="text-2xl font-semibold text-gray-700 mb-6">Buat Ujian Baru</h1>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('admin.exams.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="duration" id="duration" value="{{ old('duration') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('duration') border-red-500 @enderror" placeholder="Contoh: 120">
                @error('duration')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
